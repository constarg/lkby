    ...
    pthread_cleanup_push(keyboard_routine_cleanup_handler, src)
        ...
        while (true) {
            ...
            if (kb_event_buffer.type == EV_KEY) {
                ...
                // Fill the members.
                LKBY_INFO_KEYBOARD_NAME(&transmit_info, lkby_trans_key) = LKBY_INFO_KEYBOARD_NAME(
                    &keyboard, lkby_keyboard);
    ...
    pthread_cleanup_push(scheduler_cleanup_handler, NULL)
        ...
        while (true) {
            ...
            while (true != lkbyqueue_isempty(&LKBYQUEUE(&g_keyboard_queue))) {
                ...
                if (0 != pthread_create(&ac_kb->kb_thread, NULL, keyboard_routine, (void *) kb))
                    goto
